...
void NaturalGradientPerElementScaleComponent::Update(
  ...
  /*
   * TODO: Constructors
CuDNNConvolutionComponent::CudnnConvolutionComponent():
  UpdatableComponent(),
  {}
  */
void CuDNNConvolutionComponent::Init(
    ...
    BaseFloat param_stddev, BaseFloat bias_stddev) {
  KALDI_ASSERT((input_x_dim - filt_x_dim) % filt_x_stride == 0);
  KALDI_ASSERT((input_y_dim - filt_y_dim) % filt_y_stride == 0);
  KALDI_ASSERT((input_z_dim - filt_z_dim) % filt_z_stride == 0);
  ...
  CUDNN_SAFE_CALL(cudnnCreateConvolutionDescriptor(&convDesc_));
  CUDNN_SAFE_CALL(
    cudnnSetConvolutionDescriptor(convDesc_,
  ...
  CUDNN_SAFE_CALL(cudnnCreateFilterDescriptor(&filterDesc_));
  CUDNN_SAFE_CALL(
    cudnnSetFilterNdDescriptor(filterDesc_,
			       cudnn::GetDataType(),
			       kConvolutionDimension_,
			       filters
			       )
  ...
  delete[] strides; delete[] upscales; delete[] padding;
}

void CuDNNConvolutionComponent::InitFromConfig(ConfigLine *cfl) {
  bool ok = true;
...
void CuDNNConvolutionComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
					  const CuMatrixBase<BaseFloat> &in,
					  CuMatrixBase<BaseFloat> *out) const {

  cudnnTensorDescriptor_t inDesc;
  CUDNN_SAFE_CALL(cudnnCreateTensorDescriptor(&inDesc));
  //TODO:
  //CUDNN_SAFE_CALL(cudnnSetTensorNdDescriptor(inDesc, ...));
  cudnnTensorDescriptor_t outDesc;
  CUDNN_SAFE_CALL(cudnnCreateTensorDescriptor(&outDesc));
  CUDNN_SAFE_CALL(
  ...
  CUDNN_SAFE_CALL(cudnnDestroyTensorDescriptor(inDesc));
  CUDNN_SAFE_CALL(cudnnDestroyTensorDescriptor(outDesc));
}
